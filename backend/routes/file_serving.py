...
def get_file_content():
    ...
    try:
        ...
        return create_error_response("READ_FILE_FAILED", f"Failed to read file: {e}", 500)
...
def save_file_content():
    ...
    try:
        ...
        if dir_path:
            ...
            if not full_dir_path.startswith(root_dir):
                return create_error_response("CREATE_DIR_ACCESS_DENIED", "Cannot create directory outside project", 403)
            os.makedirs(full_dir_path, exist_ok=True)
    except Exception as e:
        return create_error_response("CREATE_DIRECTORY_FAILED", f"Failed to create directory: {e}", 500)

    safe_path = os.path.abspath(os.path.join(root_dir, filename))
    if not safe_path.startswith(root_dir):
        return create_error_response("SAVE_FILE_ACCESS_DENIED", "Access denied outside project directory", 403)
    ...
    try:
        ...
        return create_error_response("SAVE_FILE_FAILED", f"Failed to save file: {e}", 500)
...
def serve_subdir_image(subdir, filename):
    ...
    if not is_safe_path(safe_path, base_dir=images_dir_abs):
        return create_error_response("IMAGE_ACCESS_DENIED", "Access denied outside allowed image directories", 403)
...
def serve_chatroom_file(chatroom_name, filename):
    ...
    if not is_safe_path(safe_path, base_dir=chatrooms_dir_abs):
        return create_error_response("CHATROOM_FILE_ACCESS_DENIED", "Access denied outside chatrooms directory", 403)

    if '..' in filename or filename.startswith('/'):
        return create_error_response("INVALID_FILENAME_PATH", "Invalid filename path components", 403)
...
def serve_static(filename):
    ...
    if not safe_path.startswith(root_dir):
        return create_error_response("PROJECT_ACCESS_DENIED", "Access denied outside project directory", 403)
    if not os.path.exists(safe_path) or os.path.isdir(safe_path):
        return create_error_response("FILE_NOT_FOUND", "File not found", 404)
