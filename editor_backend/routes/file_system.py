...
def _scan_directory_recursive(dir_path: Path, ignore_list: set, current_base_path: str) -> Optional[List[Dict[str, Any]]]:
    ...
    for item in dir_path.iterdir():
        ...
        if item_relative_path_str in ignore_list or item.name in ignore_list:
            continue

        item_type = 'folder' if item.is_dir() else 'file'
        children_data = _scan_directory_recursive(item, ignore_list, item_relative_path_str) if item_type == 'folder' else None
        items_in_dir.append({'name': item.name, 'type': item_type, 'children': children_data, 'path': item_relative_path_str})
...
def list_project_files():
    root_items = []
    ignore_set = {'.git', '.idea', '__pycache__', '.vscode', 'node_modules', '.DS_Store', 'images', 'venv', '.venv', 'env', '.env', 'vendor'}
    ...
    for item in PROJECT_ROOT.iterdir():
        ...
        item_type = 'folder' if item.is_dir() else 'file'
        children_data = _scan_directory_recursive(item, ignore_set, item_relative_path_str) if item_type == 'folder' else None
        root_items.append({'name': item.name, 'type': item_type, 'children': children_data, 'path': item_relative_path_str})
